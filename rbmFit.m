...
for epoch = 1:maxepoch
    ...
    for batch = 1:numbatches
        ...
        %go up
        ph = logistic(data*W + classes*Wc + repmat(b,numcases,1));      %P(h|y,x)
        ...
        %go down
		negdata = logistic(nhstates*W' + repmat(c,numcases,1)); %P(x|h)
		negdatastates = negdata > rand(numcases,numdims);
		negclasses = softmaxPmtk(nhstates*Wc' + repmat(cc,numcases,1));%P(y|h)
